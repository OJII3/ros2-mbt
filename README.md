...
- `rcl_interfaces/srv/SetParameters` / `GetParameterTypes` / `GetParameters` / `ListParameters` で整数parameterを設定・型取得・値取得・列挙する `examples/parameter_client`
...
Parameter相互運用テストは`rclpy` nodeの標準parameter serviceを使い、MoonBitから`SetParameters`で整数parameterを更新し、`GetParameterTypes`で型、`GetParameters`で値を取得し、`ListParameters`で列挙します。
