...
- primitive `.msg` の parser / MoonBit CDR codec generator（固定配列・unbounded sequence 対応、`rosidl`）
...
ROS message の定数・bounded sequence・nested type codegen、Cyclone DDS/Fast DDSとの実通信試験は未実装です。
...
5. `geometry_msgs/Twist`などのROS message codec（primitive・固定配列・sequence codegen は実装済み、nested type は未実装）
