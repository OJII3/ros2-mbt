...
- primitive / scalar constant `.msg` の parser / MoonBit CDR codec generator（固定配列・unbounded sequence 対応、`rosidl`）
...
ROS message の bounded sequence・nested type codegen、Cyclone DDS/Fast DDSとの実通信試験は未実装です。
...
5. `geometry_msgs/Twist`などのROS message codec（primitive・scalar constants・固定配列・sequence codegen は実装済み、bounded/nested type は未実装）
